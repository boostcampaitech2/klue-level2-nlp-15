...
train_dir = '../dataset/train/train_fixed.csv'
...
class Data:
    ...
    def __repr__(self):
        ...
        return self.sentence.replace(sword,f"[SUB]{sword}[/SUB]").replace(oword,f"[OBJ]{oword}[/OBJ]")
...
def split_dataset(ratio, train_dir):
    ...
    for label,data_list in label2data.items():
        ...
        for i,data in enumerate(data_list):
            if i < len(data_list)*ratio: valid_dataset.append(data)
            else: train_dataset.append(data)
...
def tokenize_dataset(dataset,tokenizer,train=True, bi=False):
    ...
    sub_token_indexes = []
    obj_token_indexes = []
    for tokens, (sub_token_id, obj_token_id) in zip(tokenized_sentences['input_ids'],entity_token_ids):
        ...
        try:
            ...
            print(tokenizer.decode(tokens),tokenizer.decode([sub_token_id,obj_token_id]))
            continue
