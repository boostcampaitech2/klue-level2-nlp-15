...
class AngularPenaltySMLoss(nn.Module):
    ...
    def forward(self, x, label):
        ...
        assert torch.max(label) < self.out_features
        ...
        if self.loss_type == 'arcface':
            numerator = self.s * torch.cos(torch.acos(torch.clamp(torch.diagonal(wf.transpose(0, 1)[label]), -1.+self.eps, 1-self.eps)) + self.m)
        if self.loss_type == 'sphereface':
            numerator = self.s * torch.cos(self.m * torch.acos(torch.clamp(torch.diagonal(wf.transpose(0, 1)[label]), -1.+self.eps, 1-self.eps)))
