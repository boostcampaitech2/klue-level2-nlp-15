...
def tokenized_dataset(dataset, tokenizer):
    ...
    max_length = 256
    sent_ = sent_[:max_length - 2]
    senttokens = tokenizer.tokenize(sent_)
    input_ids = tokenizer.convert_tokens_to_ids(senttokens)
    return input_ids, ss, es
...
def collate_fn(batch):
    ...
    output = (input_ids, input_mask, labels, ss, es)
    return output
...
def processor(tokenizer, dataset):
    '''
    ...
    for i in range(len(dataset)):
        input_ids, new_ss, new_es = tokenized_dataset(dataset.iloc[i], tokenizer)
        label = labels[i]
        feature = {
            'input_ids' : input_ids,
            'labels' : label,
